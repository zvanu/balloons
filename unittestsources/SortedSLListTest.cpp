...
#include "..\balloons\dtlibrary\SortedSLList.h"

using namespace Microsoft::VisualStudio::CppUnitTestFramework;

namespace unittest
{		
	TEST_CLASS(SortedSLListTest)
	{
	public:
		TEST_METHOD(addTest)
		{
			Containers::SortedSLList<int> sList;
			int index;
		...
		{
			Containers::SortedSLList<int> sList;
			Containers::SortedSLList<int>::Iterator it;

			it = sList.begin();
			Assert::IsTrue(false == it.hasNext());
			Assert::IsTrue(false == it.isValid());
			sList.add(0);
			sList.add(1);
			sList.add(2);
			it = sList.begin();
